...
The workspace separates durable concepts so each has one clear job:

- **Canon** defines shared engineering principles.
- **Identity** personalizes how the assistant collaborates with an engineer.
- **Skills** encode reusable engineering procedures.
- **Loops** compose skills into repeatable, stateful workflows.
- **Adapters** expose the canon and supported workflows through tool-native formats.
- **Validation** prevents drift and invalid references.

The methodology defines judgment. Skills define reusable procedures. Loops define
repeatable coordination. Tools remain delivery mechanisms. The methodology is
still the product: loops are an initial, report-only canonical layer, not an
automation platform or an autonomous runtime.
