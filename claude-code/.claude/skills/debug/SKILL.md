...
1. **Reproduce.** Get a deterministic, minimal reproduction first. Capture exact inputs, environment, and the failing path. No repro means you cannot verify a fix — say so explicitly and treat any fix as unconfirmed.
...
- Constructive disagreement: if the user's proposed fix targets a symptom, say so and point at the cause.
